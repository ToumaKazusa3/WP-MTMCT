...
import sys
sys.path.append('/home/shensj/code/WP-MTMCT/mytracker/mot/FairMOT/src/lib')

# from tracker.multitracker import JDETracker, joint_stracks, STrack
from tracker.multitracker_new import JDETracker_new, joint_stracks, STrack
from tracking_utils import visualization as vis
from tracking_utils.log import logger
from tracking_utils.timer import Timer
from tracking_utils.evaluation import Evaluator
import datasets.dataset.jde as datasets
from lib.utils.utils import DataProcesser, DataPacker
...
def eval_seq(opt, dataloader, data_type, result_filename, det_filename, camid,
             reid_model_path, save_dir=None, show_image=True, frame_rate=30):
    if save_dir:
        mkdir_if_missing(save_dir)
    # 初始化tracker
    tracker = JDETracker_new(opt, frame_rate=frame_rate, det_path=det_filename,
                             reid_model_path=reid_model_path, camid=camid)
    timer = Timer()
    results = []
    frame_id = 0
...
def track_main(opt, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo',
    ...
    save_feature_path = crop_root / 'feature.pkl'
    ...
    for seq in seqs:
        ...
        nf, ta, tc, feature = eval_seq(opt, dataloader, data_type, result_filename, opt.det_filename, camid,
                              opt.reid_model_path, save_dir=output_dir, show_image=show_image, frame_rate=frame_rate)
    ...
    if count_dataset_info:
        info = DataProcesser.count_dataset_info(tracklet_dir=crop_root, cam_num=6, verbose=True, save_info=True)
    if save_feature:
        idx_ava = [i.keys() for i in info]
        feature_ava = feature_all[idx_ava]
        m, n = feature_ava.size(0), feature_ava.size(0)
        distmat = torch.pow(feature_ava, 2).sum(dim=1, keepdim=True).expand(m, n) + \
                  torch.pow(feature_ava, 2).sum(dim=1, keepdim=True).expand(n, m).t()
        distmat.addmm_(1, -2, feature_ava, feature_ava.t())
        feature_all = np.asarray(feature_all)
        feature_ava = np.asarray(feature_ava)
        distmat = np.asarray(distmat)
        fea_dict = {'feature_all':feature_all, 'feature_ava':feature_ava, 'distmat':distmat}
        DataPacker.dump(fea_dict, save_feature_path)
...
if __name__ == '__main__':
    ...
    data_root = Path('/data4/shensj/datasets/my_files/gps/GPSReID/MOT/GPSReID/test/')
