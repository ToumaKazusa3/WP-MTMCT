...
from tracker.multitracker_new import JDETracker_new
...
def eval_seq(opt, dataloader, data_type, result_filename, det_filename, save_dir=None, show_image=True, frame_rate=30):
    if save_dir:
    ...
    for path, img00, imgList, img0List in dataloader:
        ...
        # blob = torch.from_numpy(img00).cuda().unsqueeze(0)
...
def track_main(opt, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo', exp_name_new='demo',
    ...
    result_root_new = data_root / '..' / 'results' / exp_name_new
    save_feature_path = result_root / 'feature.pkl'
    crop_root =  data_root / '..' / 'crop_images' / exp_name_new
    ...
    for seq in seqs:
        camid = int(seq[-1])
        output_dir = (data_root / '..' / 'outputs' / exp_name_new / seq) if save_images or save_videos else None
        result_filename = result_root_new / '{}.txt'.format(seq)
        result_filename_refine = result_root_new / '{}_refine_v2.txt'.format(seq)
        det_filename = result_root / '{}_update_with_gps.txt'.format(seq)
        ...
        # nf, ta, tc, feature = eval_seq(opt, dataloader, data_type, result_filename, det_filename,
...
if __name__ == '__main__':
    ...
    exp_name_new = opt.exp_name_new

    track_main(opt,
         data_root=data_root,
         seqs=seqs,
         exp_name=exp_name,
         exp_name_new=exp_name_new,
         show_image=False,
         save_images=True,
         save_videos=False,
         save_crop_images=False,
         count_dataset_info=True,
         generate_refine_result=False,
         save_feature=False,)
